...
This way, the Worker holds the credentials and delegates to Flow, falling back to the Cloudflare Durable Object + TURN setup only when that fails.
As a result, normal operation only costs Worker requests, and when Flow goes down it falls back to Cloudflare — low-cost HA.
